...
func handleSpecificTop(state State, msg message.MsgPrivate) bool {
	if strings.ToLower(msg.User) != state.Conf.Topper &&
		strings.ToLower(msg.User) != state.Conf.Admin {
		return false
	}
	lower_msg := strings.ToLower(msg.Msg)
	if !strings.HasPrefix(lower_msg, "top") {
		return false
	}
...
func handleRollback(state State, msg message.MsgPrivate) bool {
	if strings.ToLower(msg.Msg) != "rollback" {
		return false
	}
	if strings.ToLower(msg.User) != state.Conf.Admin {
		return false
...
func handleTriggers(state State, msg message.MsgPrivate) bool {
	for _, trigger := range state.Conf.Triggers {
		if handleTrigger(state, msg, trigger) {
			return true
		}
	}
	return false
...
func handleMetapi(state State, msg message.MsgPrivate) bool {
	...
	if !strings.HasPrefix(strings.ToLower(msg.Msg), "metapi") {
		return false
	}
	log.Printf("Passed for %s", msg.Msg)
	num, err := strconv.ParseInt(msg.Msg[7:], 10, 64)
	if err != nil {
		state.ResponseChannel <- message.MsgSend{msg.Response(), fmt.Sprintf("Expect an int32 to search, error was %q", err)}
		return true
	} else {
		state.ResponseChannel <- message.MsgSend{msg.Response(), "Launching big real data time calculation on the claoud"}
